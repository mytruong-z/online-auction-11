...
const Order = (props) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/api/nguoi-ban/don-hang/thong-ke-don-hang`, {
        headers: {
  ...
  const renderButton = status => {
    if(status === 1){
      return <>
         <button class="btn btn-success btn-action">Giao</button>
    ...
    <div>
      <div className="section-title m-4">
        ...
        <table class="table">
            ...
            <tbody>
                { data.length !== 0 && data.map(don_hang => {
                    return <tr>
                    <td>
                      <Link to={`/san-pham/${don_hang.path}`}>{don_hang.ten}</Link>
                    </td>
                    <td>
                    <Link to={`/nguoi-dung/thong-tin/${don_hang.id_nguoi_dung}`}>{don_hang.ho_ten}</Link>
                    </td>
                    <td>{don_hang.gia_mua} $</td>
                    <td>{new Date(don_hang.ngay_dat_hang).toLocaleString("en-US")}</td>
                    <td>
                        {renderTrangThai(don_hang.status, don_hang.ten_trang_thai)}
                    </td>
                    <td>
                        {renderButton(don_hang.status)}
